...
if ($client_id_to_edit <= 0) {
    $_SESSION['error_message'] = "Invalid client ID specified for editing.";
...
if ($conn) {
    ...
    if ($stmt_fetch) {
        ...
        if ($result_fetch->num_rows === 1) {
            ...
            $_SESSION['error_message'] = "Client not found for editing (ID: $client_id_to_edit).";
            $stmt_fetch->close();
            redirect('admin/clients/');
        }
        $stmt_fetch->close();
    } else {
        $_SESSION['error_message'] = "Error preparing statement to fetch client: " . $conn->error;
        redirect('admin/clients/');
    }
} else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $private_key_posted = sanitize_input($_POST['private_key']);
    ...
    $private_key_hash_for_db = $client_data['private_key']; // Keep old hash by default
    if (!empty($private_key_posted)) {
        if (strlen($private_key_posted) < 6) {
            $errors['private_key'] = "Private key must be at least 6 characters long.";
        } else {
            $private_key_hash_for_db = password_hash($private_key_posted, PASSWORD_DEFAULT);
        }
    }
    ...
    if (empty($errors)) {
        ...
        if ($conn_update) {
            $sql = "UPDATE clients SET 
                        ...
                        client_account_status = ?, private_key = ?, updated_at = CURRENT_TIMESTAMP
                    WHERE client_id = ?";
            ...
            if ($stmt_update) {
                ...
                $stmt_update->bind_param("ssssssssssi",
                    ...
                    $client_account_status_posted, $private_key_hash_for_db, $client_id_to_edit
                );

                if ($stmt_update->execute()) {
                    $_SESSION['success_message'] = "Client '" . htmlspecialchars($first_name . ' ' . $last_name) . "' updated successfully!";
                    if ($new_password_hash_for_db !== $client_data['password_hash'] && !empty($client_portal_password)) {
                         $_SESSION['success_message'] .= " Client portal password has been changed.";
                    }
                    if ($client_account_status_posted !== $client_data['client_account_status']) {
                         $_SESSION['success_message'] .= " Client account status updated to '" . htmlspecialchars($client_account_status_posted) . "'.";
                    }
                    if ($private_key_hash_for_db !== $client_data['private_key'] && !empty($private_key_posted)) {
                         $_SESSION['success_message'] .= " Private key has been updated.";
                    }
                    redirect('admin/clients/view_client.php?id=' . $client_id_to_edit);
                } else {
                    $_SESSION['error_message'] = "Error updating client: " . $stmt_update->error;
    ...
    <form action="edit_client.php?id=<?php echo $client_id_to_edit; ?>" method="POST" class="space-y-6">
        ...
        <div class="bg-slate-900 border border-white/5 rounded-2xl overflow-hidden shadow-2xl">
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    <label for="private_key" class="block text-xs font-bold uppercase text-slate-400 mb-2 tracking-widest">Private Key (for Portal Access)</label>
                    <div class="flex gap-2">
                        <input type="password" name="private_key" id="private_key"
                               class="flex-1 px-4 py-2.5 bg-slate-800 border <?php echo isset($errors['private_key']) ? 'border-red-500' : 'border-slate-700'; ?> rounded-lg text-slate-100 placeholder-slate-600 focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all">
                        <button type="button" id="generate_key_btn" class="px-4 py-2.5 bg-slate-700 hover:bg-slate-600 text-slate-300 hover:text-white rounded-lg transition-all duration-300 text-sm font-semibold" title="Generate a secure random key">
                            <i class="fas fa-key"></i>
                        </button>
                        <button type="button" id="toggle_key_btn" class="px-4 py-2.5 bg-slate-700 hover:bg-slate-600 text-slate-300 hover:text-white rounded-lg transition-all duration-300 text-sm font-semibold" title="Toggle visibility">
                            <i class="fas fa-eye"></i>
                        </button>
                    </div>
                    <p class="text-xs text-slate-400 mt-1">Min. 6 characters. Click the key icon to generate a secure random key, eye icon to show/hide. This key will be required after client login to access case information. Leave blank to keep current key (if set).</p>
                    <?php if (isset($errors['private_key'])): ?><p class="text-red-400 text-xs mt-1.5 flex items-center gap-1"><i class="fas fa-times-circle"></i><?php echo htmlspecialchars($errors['private_key']); ?></p><?php endif; ?>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    const generateKeyBtn = document.getElementById('generate_key_btn');
    const toggleKeyBtn = document.getElementById('toggle_key_btn');
    const privateKeyInput = document.getElementById('private_key');

    generateKeyBtn.addEventListener('click', function() {
        // Generate a secure 12-character random key
        ...
        const originalIcon = generateKeyBtn.innerHTML;
        generateKeyBtn.innerHTML = '<i class="fas fa-check"></i>';
        generateKeyBtn.classList.add('bg-green-600', 'text-white');
        generateKeyBtn.classList.remove('bg-slate-700', 'hover:bg-slate-600');

        setTimeout(() => {
            generateKeyBtn.innerHTML = originalIcon;
            generateKeyBtn.classList.remove('bg-green-600', 'text-white');
            generateKeyBtn.classList.add('bg-slate-700', 'hover:bg-slate-600');
        }, 1000);
    });
    ...
    toggleKeyBtn.addEventListener('click', function() {
        ...
        const originalIcon = toggleKeyBtn.innerHTML;
        ...
        toggleKeyBtn.classList.add(isPassword ? 'bg-blue-600' : 'bg-slate-700');
        toggleKeyBtn.classList.remove(isPassword ? 'bg-slate-700' : 'bg-blue-600');

        setTimeout(() => {
            toggleKeyBtn.classList.remove('bg-blue-600');
            toggleKeyBtn.classList.add('bg-slate-700');
        }, 300);
    });
});
</script>
