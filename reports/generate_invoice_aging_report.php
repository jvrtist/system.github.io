<?php
// reports/generate_invoice_aging_report.php
...
$clients_list = [];
$statuses = ['Draft', 'Sent', 'Partially Paid', 'Paid', 'Overdue', 'Void', 'Cancelled'];
...
if ($_SERVER["REQUEST_METHOD"] === "GET" && isset($_GET['generate_report'])) {
    ...
    $sql .= " ORDER BY i.due_date ASC, i.invoice_date DESC";
...
<div class="space-y-10">
    <header class="flex flex-col md:flex-row md:items-center justify-between gap-4 no-print">
        <div>
            <h1 class="text-3xl font-black text-white uppercase tracking-tighter">Invoice <span class="text-primary">Aging</span></h1>
            <p class="text-sm text-slate-500 tech-mono">Track outstanding balances and overdue exposure.</p>
        </div>
    ...
    <form action="generate_invoice_aging_report.php" method="GET" class="bg-slate-900/50 p-6 rounded-2xl border border-white/5 no-print">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-xs font-black uppercase tracking-widest text-slate-400">Report Filters</h3>
            <span class="text-[10px] tech-mono text-slate-600 uppercase">Billing Module</span>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 items-end">
            ...
            <div class="space-y-1">
                <label for="filter_status" class="text-[10px] font-black uppercase text-slate-500 ml-1">Status</label>
            ...
            <div class="space-y-1">
                <label for="filter_date_start" class="text-[10px] font-black uppercase text-slate-500 ml-1">Invoice Date From</label>
            ...
            <div class="space-y-1">
                <label for="filter_date_end" class="text-[10px] font-black uppercase text-slate-500 ml-1">Invoice Date Until</label>
    ...
    <?php if ($report_generated): ?>
        <section class="print-area bg-slate-900 rounded-2xl border border-white/5 overflow-hidden shadow-2xl">
            <div class="print-header">
                <h2 class="text-2xl font-bold"><?php echo SITE_NAME; ?> - Invoice Aging Report</h2>
            ...
            <div class="px-8 py-6 border-b border-white/5 flex items-center justify-between bg-white/[0.02] no-print">
                ...
                <div class="flex items-center gap-4">
                    <span class="text-[10px] tech-mono text-slate-600 uppercase"><?php echo count($report_data); ?> Invoices</span>
            ...
            <?php if (!empty($report_data)): ?>
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-white/5">
                                <th class="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400">Invoice</th>
                                ...
                                <th class="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400">Invoice Date</th>
                                ...
                                <th class="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400">Days Overdue</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-white/5">
                            <?php foreach ($report_data as $row): ?>
                                <?php
                                    $days_overdue = $row['due_date'] ? (int)$row['days_overdue'] : null;
                                    $overdue_class = ($days_overdue !== null && $days_overdue > 0 && $row['status'] !== 'Paid') ? 'text-red-400 font-black' : 'text-slate-500';
                                ?>
                                <tr class="hover:bg-white/[0.02] transition-colors">
                                    <td class="px-6 py-4 text-xs font-bold text-primary">#<?php echo htmlspecialchars($row['invoice_number']); ?></td>
                                    ...
                                    <td class="px-6 py-4 text-[10px] tech-mono uppercase text-slate-500"><?php echo htmlspecialchars($row['status']); ?></td>
                                    <td class="px-6 py-4 text-[10px] tech-mono text-slate-500 uppercase"><?php echo date("Y-m-d", strtotime($row['invoice_date'])); ?></td>
                                    <td class="px-6 py-4 text-[10px] tech-mono text-slate-500 uppercase"><?php echo $row['due_date'] ? date("Y-m-d", strtotime($row['due_date'])) : 'N/A'; ?></td>
                                    <td class="px-6 py-4 text-xs text-slate-300 font-bold">R<?php echo number_format((float)$row['balance_due'], 2); ?></td>
                                    <td class="px-6 py-4 text-[10px] tech-mono uppercase <?php echo $overdue_class; ?>">
                                        <?php echo ($days_overdue !== null && $days_overdue > 0) ? $days_overdue : '0'; ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
            ...
            <?php else: ?>
                <div class="py-16 text-center">
                    ...
                    <p class="text-[10px] tech-mono text-slate-600 uppercase tracking-widest">No invoices found for the selected criteria.</p>
                </div>
            <?php endif; ?>
        </section>
    <?php endif; ?>
